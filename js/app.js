...
function globalStress(scores){const positive=scores.filter(s=>Number.isFinite(s)&&s>0);if(!positive.length)return 0;return Math.min(5,Math.sqrt(positive.reduce((sum,s)=>sum+s*s,0)))}
...
async function showNuisancesAt(lon, lat) {
  ...
  const currentHour=Math.max(6,Math.min(23,new Date().getHours())),dailyMax=Math.max(...(pressureData.stations||[]).map(s=>s.weekdayAverage||0),1),hourlyMax=Math.max(...(pressureData.stations||[]).map(s=>s.hourly?.[currentHour]||0),1),stationHits=(pressureData.stations||[]).map(s=>{const ratio=Math.sqrt((s.weekdayAverage||0)/dailyMax),radius=800+700*ratio,d=distanceMeters(lon,lat,s.lon,s.lat);return{...s,d,radius,ratio,hourValue:s.hourly?.[currentHour]||0}}).filter(s=>s.d<=s.radius).sort((a,b)=>a.d-b.d),busHits=(pressureData.stops||[]).filter(s=>distanceMeters(lon,lat,s.lon,s.lat)<=300),mobility5=Math.min(5,(stationHits[0]?.hourValue||0)/hourlyMax*4.5+Math.min(.5,busHits.length*.08));
  const equipmentHits=(pressureData.equipment||[]).filter(p=>distanceMeters(lon,lat,p.lon,p.lat)<=750),equipmentScore=Math.min(5,5*Math.log1p(equipmentHits.length)/Math.log(81));
  ...
  const aircraftNear=movingAircraft.map(a=>{const p=a.marker.getLatLng();return distanceMeters(lon,lat,p.lng,p.lat)}).filter(d=>d<=5000),aircraftScore=Math.min(5,aircraftNear.length),artificialScore=artificial==null?null:artificial?1.5:0;
  const rows=[
    {name:"Mobilité & fréquentation",score:mobility5,value:stationHits[0]?`${stationHits[0].name} · ${fmt(stationHits[0].hourValue)} validations entre ${currentHour} h et ${currentHour+1} h`:`${busHits.length} arrêt(s) à moins de 300 m`,detail:`${stationHits.length} gare(s) dans leur aire d’influence · ${busHits.length} arrêt(s) proches`,source:"IDFM · profil horaire du jour ouvré moyen · heure actuelle"},
    {name:"Équipements & services",score:equipmentScore,value:`${equipmentHits.length} lieux à moins de 750 m`,detail:"Masse cumulée, sans distinction de catégorie · échelle logarithmique",source:"BPE Insee + OpenStreetMap · millésimes publiés"},
    {name:"Pression routière",score:roadScore,value:`${roadDb?`${roadDb}–${roadDb+5} dB(A) · `:""}${traffic?`trafic ${traffic.label}`:trafficFresh?"trafic local non lu":"trafic temps réel indisponible"}`,detail:road[0]?`Classement sonore catégorie ${road[0].cat} · arrêté n°17-146${traffic?" · état actuel intégré":""}`:"Hors secteur routier classé",source:`DDT 95 · Bruitparif · ${trafficFresh?"Sytadin actuel":"Sytadin ancien : exclu de la note"}`},
    {name:"Pression ferroviaire",score:railScore,value:railDb?`${railDb}–${railDb+5} dB(A)`:"Pas de niveau sonore local",detail:rail[0]?`Classement sonore catégorie ${rail[0].cat} · arrêté n°16249`:"Hors secteur ferroviaire classé",source:"DDT 95 · SNCF/RATP/SGP · classement réglementaire"},
    {name:"Air & bruit",score:airScore,value:localAirNoise?`Coexposition locale : classe ${localAirNoise}/5`:"Classe locale indisponible",detail:air?`Commune : air dégradé ${fmt(air.air_degrade_pct,1)} % · bruit dégradé ${fmt(air.bruit_degrade_pct,1)} %`:"Contexte communal indisponible",source:"Airparif + Bruitparif · raster local 2024"},
    {name:"Météo & extrêmes",score:climateScore,value:temp?`${fmt(temp.temperature,1)} °C · ressenti ${fmt(temp.apparent,1)} °C · ${weather.detail}`:"Observation indisponible",detail:heatExtreme&&heatDay?`Extrême thermique : aléa morphoclimatique ${heatDay}/5 intégré`:"Température ordinaire : aucun stress thermique ajouté",source:`Open-Meteo · observation ${temp?.time||"actuelle"} · IPR`},
    {name:"Artificialisation",score:artificialScore,value:artificial==null?"Lecture indisponible":artificial?"Sol artificialisé":"Sol non artificialisé",detail:"Pression de fond limitée à 1,5/5 dans le cumul",source:"OCS GE IGN · lecture au point exact"},
    {name:"Trafic aérien",score:aircraftScore,value:`${aircraftNear.length} aéronef(s) à moins de 5 km`,detail:"1 niveau de stress par aéronef proche, plafonné à 5",source:`ADSB.lol · positions en direct · ${new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}`}
  ];
...
function openStressPdf(stress){const {jsPDF}=window.jspdf||{};if(!jsPDF)return;const doc=new jsPDF({unit:"mm",format:"a4"}),c=stressColor(stress.score),rgb=c.match(/\w\w/g).map(x=>parseInt(x,16));doc.setFillColor(...rgb);doc.rect(0,0,210,48,"F");doc.setTextColor(255);doc.setFont("helvetica","bold");doc.setFontSize(9);doc.text("DDT DU VAL-D'OISE · CARTE DE STRESS",16,12);doc.setFontSize(23);doc.text(stress.commune||"Zone analysée",16,27);doc.setFontSize(10);doc.text(stress.nearestAddress||"Adresse BAN indisponible",16,36,{maxWidth:120});doc.setFontSize(19);doc.text(displayScore(stress.score),155,25);doc.setFontSize(10);doc.text(stressLabel(stress.score),155,34);doc.setTextColor(22,32,43);doc.setFont("helvetica","normal");doc.setFontSize(8);doc.text(`Point exact : ${stress.coordinates.lat.toFixed(6)}, ${stress.coordinates.lon.toFixed(6)} · calcul ${new Date(stress.generatedAt).toLocaleString("fr-FR")}`,16,58);doc.autoTable({startY:66,head:[["PRESSION ET NOTE","MESURE OBSERVÉE","RÈGLE, CLASSEMENT ET SOURCE"]],body:stress.criteria.map(r=>[`${r.name}\n${displayScore(r.score)} · ${stressLabel(r.score)}`,r.value,`${r.detail}\n${r.source||"Donnée publique"}`]),styles:{fontSize:7.4,cellPadding:3.2,valign:"middle",lineColor:[225,229,235],lineWidth:.2},headStyles:{fillColor:[0,0,145]},columnStyles:{0:{cellWidth:43,fontStyle:"bold"},1:{cellWidth:59},2:{cellWidth:82}},didParseCell:d=>{if(d.section==="body"&&d.column.index===0){const col=stressColor(stress.criteria[d.row.index].score||0).match(/\w\w/g).map(x=>parseInt(x,16));d.cell.styles.textColor=col}}});let y=doc.lastAutoTable.finalY+9;doc.setFont("helvetica","bold");doc.setFontSize(9);doc.text("Comment est calculé le niveau ?",16,y);doc.setFont("helvetica","normal");doc.setFontSize(8);doc.text("Les pressions simultanées se cumulent par racine carrée de la somme des notes au carré, avec un plafond de 5. La mesure, le classement et la fraîcheur sont affichés sur la même ligne que chaque note. Indice exploratoire, non sanitaire.",16,y+6,{maxWidth:178});doc.setTextColor(90);doc.text("Carte de stress · données publiques · DDT du Val-d'Oise",16,287);const url=URL.createObjectURL(doc.output("blob"));window.open(url,"_blank","noopener")}
