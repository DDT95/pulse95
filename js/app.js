...
function stressCard(row){return `<article class="stress-card" style="--stress:${stressColor(row.score||0)}"><header><div><small>${esc(row.name)}</small><strong>${displayScore(row.score)}</strong></div><em>${row.score==null?"Non calculable":stressLabel(row.score)}</em></header><p class="stress-why"><b>POURQUOI CETTE NOTE ?</b>${esc(row.why||row.detail)}</p><div class="stress-evidence"><p><b>MESURE</b><span>${esc(row.value)}</span></p><p><b>RÈGLE / CLASSEMENT</b><span>${esc(row.detail)}</span></p><p><b>SOURCE · FRAÎCHEUR</b><span>${esc(row.source||"Donnée publique")}</span></p></div></article>`}
...
async function showNuisancesAt(lon, lat) {
  ...
  const rows=[
    ...
    {name:"Artificialisation",score:artificialScore,value:artificial==null?"Lecture indisponible":artificial?"Sol artificialisé":"Sol non artificialisé",why:artificial?"Un sol artificialisé accroît la pression de fond, mais sa contribution est volontairement limitée car il ne constitue pas à lui seul une nuisance immédiate.":"Le sol n’est pas identifié comme artificialisé au point exact.",detail:"Pression de fond limitée à 1,5/5 dans le cumul",source:"OCS GE IGN · lecture au point exact"},
