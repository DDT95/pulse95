...
function stressLabel(score){return score>=80?"Très fort":score>=60?"Fort":score>=40?"Marqué":score>=20?"Modéré":"Faible"}
function stressCard(name,score,value,detail){const s=score==null?null:Math.round(score);return `<div class="stress-card"><div><small>${esc(name)}</small><strong>${s==null?"—":s+" / 100"}</strong><em>${s==null?"Non calculable":stressLabel(s)}</em></div><span>${esc(value)}</span><p>${esc(detail)}</p></div>`}
...
async function sampleArtificial(lon,lat){try{const z=15,n=2**z,x=Math.floor((lon+180)/360*n),y=Math.floor((1-Math.asinh(Math.tan(lat*Math.PI/180))/Math.PI)/2*n),px=Math.floor(((lon+180)/360*n-x)*256),py=Math.floor(((1-Math.asinh(Math.tan(lat*Math.PI/180))/Math.PI)/2*n-y)*256),url=`https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=OCSGE.ARTIF.2024-2026&STYLE=normal&TILEMATRIXSET=PM&FORMAT=image/png&TILEMATRIX=${z}&TILEROW=${y}&TILECOL=${x}`,blob=await fetch(url).then(r=>r.blob()),bmp=await createImageBitmap(blob),c=document.createElement("canvas");c.width=256;c.height=256;c.getContext("2d").drawImage(bmp,0,0);const d=c.getContext("2d").getImageData(px,py,1,1).data;return d[3]>35}catch{return null}}
async function showNuisancesAt(lon, lat) {
  ...
  const noiseHtml =
    roadDb || railDb
      ...
      : "";
  const commune=communeAt(lon,lat),code=String(commune?.properties?.code||commune?.properties?.insee||commune?.properties?.INSEE_COM||""),air=state.stats[code],ban=await reverseBan(lon,lat),artificial=await sampleArtificial(lon,lat);
  const dailyMax=Math.max(...(pressureData.stations||[]).map(s=>s.weekdayAverage||0),1),stationHits=(pressureData.stations||[]).map(s=>{const ratio=Math.sqrt((s.weekdayAverage||0)/dailyMax),radius=800+700*ratio,d=distanceMeters(lon,lat,s.lon,s.lat);return{...s,d,radius,ratio}}).filter(s=>s.d<=s.radius).sort((a,b)=>a.d-b.d),busHits=(pressureData.stops||[]).filter(s=>distanceMeters(lon,lat,s.lon,s.lat)<=300),mobilityScore=Math.min(100,(stationHits[0]?.ratio||0)*72+Math.min(28,busHits.length*3.5));
  const equipmentHits=(pressureData.equipment||[]).filter(p=>distanceMeters(lon,lat,p.lon,p.lat)<=750),equipmentScore=Math.min(100,100*Math.log1p(equipmentHits.length)/Math.log(81));
  const roadReg=road[0]?.cat?Math.max(0,(6-road[0].cat)*20):0,traffic=sampleTrafficAt(lon,lat),roadScore=Math.max(roadDb?Math.max(0,(roadDb-40)/35*100):0,roadReg,traffic?.score||0),railReg=rail[0]?.cat?Math.max(0,(6-rail[0].cat)*20):0,railScore=Math.max(railDb?Math.max(0,(railDb-55)/20*100):0,railReg);
  const airScore=air?Math.max(Number(air.air_degrade_pct)||0,Number(air.bruit_degrade_pct)||0):null,heatFeature=featureAtPoint(lon,lat,pressureData.heat),heatDay=heatFeature?Number(heatFeature.properties.day)+1:null,temp=(pressureData.temperaturePoints||[]).map(p=>({...p,d:distanceMeters(lon,lat,p.lon,p.lat)})).sort((a,b)=>a.d-b.d)[0],climateScore=heatDay==null?null:Math.max(0,Math.min(100,heatDay*20));
  const aircraftNear=movingAircraft.map(a=>{const p=a.marker.getLatLng();return distanceMeters(lon,lat,p.lng,p.lat)}).filter(d=>d<=5000),aircraftScore=Math.min(100,aircraftNear.length*20),scores=[mobilityScore,equipmentScore,roadScore,railScore,airScore,climateScore,artificial==null?null:artificial?100:0,aircraftScore].filter(Number.isFinite),global=Math.round(scores.reduce((a,b)=>a+b,0)/(scores.length||1));
  const stress={generatedAt:new Date().toISOString(),coordinates:{lat,lon},address:ban?.label||null,commune:air?.nom||ban?.city||null,score:global,criteria:{mobility:mobilityScore,equipment:equipmentScore,road:roadScore,rail:railScore,airNoise:airScore,climate:climateScore,artificialization:artificial==null?null:artificial?100:0,aircraft:aircraftScore},raw:{stations:stationHits.slice(0,3).map(s=>({name:s.name,distance_m:Math.round(s.d),validations_day:s.weekdayAverage})),busStops:busHits.length,equipment:equipmentHits.length,roadDb,railDb,traffic:traffic?.label||null,temperature_c:temp?.temperature??null,apparentTemperature_c:temp?.apparent??null,aircraftWithin5km:aircraftNear.length}};state.currentStress=stress;
  const cards=stressCard("Mobilité & fréquentation",mobilityScore,`${stationHits.length} gare(s) · ${busHits.length} arrêt(s)`,stationHits[0]?`${stationHits[0].name} · ${fmt(stationHits[0].weekdayAverage)} validations/jour`:"Aucune gare dans son aire d’influence")+stressCard("Équipements & services",equipmentScore,`${equipmentHits.length} lieux à moins de 750 m`,"Concentration cumulative, sans pondération par type")+stressCard("Pression routière",roadScore,traffic?`Trafic ${traffic.label}`:(roadDb?`${roadDb}–${roadDb+5} dB(A)`:"Pas de mesure locale"),road[0]?`Classement sonore catégorie ${road[0].cat}`:"Hors secteur routier classé")+stressCard("Pression ferroviaire",railScore,railDb?`${railDb}–${railDb+5} dB(A)`:"Pas de niveau local",rail[0]?`Classement sonore catégorie ${rail[0].cat}`:"Hors secteur ferroviaire classé")+stressCard("Air & bruit communal",airScore,air?`Air ${fmt(air.air_degrade_pct,1)} % · bruit ${fmt(air.bruit_degrade_pct,1)} %`:"Donnée indisponible","Part de population communale en situation dégradée")+stressCard("Chaleur & température",climateScore,temp?`${fmt(temp.temperature,1)} °C · ressenti ${fmt(temp.apparent,1)} °C`:"Température indisponible",heatDay?`Aléa morphoclimatique ${heatDay}/5`:"Aléa local indisponible")+stressCard("Artificialisation",artificial==null?null:artificial?100:0,artificial==null?"Lecture indisponible":artificial?"Sol artificialisé":"Sol non artificialisé","OCS GE IGN au point sélectionné")+stressCard("Trafic aérien",aircraftScore,`${aircraftNear.length} aéronef(s) à moins de 5 km`,"Positions ADS-B observées maintenant");
  openDetail(`<span class="detail-tag">FICHE DE STRESS TERRITORIAL</span><h2>${esc(ban?.label||air?.nom||"Zone sélectionnée")}</h2><div class="stress-global"><small>Note de stress</small><strong>${global}<span>/100</span></strong><em>${stressLabel(global)}</em></div><div class="stress-grid">${cards}</div><details><summary>Données réglementaires détaillées</summary>${regHtml}${noiseHtml}</details><p class="flag-note">Note = moyenne simple des critères disponibles. Elle décrit une pression territoriale relative, pas un risque sanitaire individuel. Adresse issue de la BAN via le géocodeur Géoplateforme.</p><div class="stress-actions"><button id="printStress">Imprimer / PDF</button><button id="exportStress">Exporter JSON</button></div>`);
  $("printStress").onclick=()=>window.print();$("exportStress").onclick=()=>{const blob=new Blob([JSON.stringify(state.currentStress,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`pulse95-stress-${lat.toFixed(5)}-${lon.toFixed(5)}.json`;a.click();URL.revokeObjectURL(a.href)};
}
...
async function loadTemperatures(){
  try{
    ...
    const url=`https://api.open-meteo.com/v1/forecast?latitude=${points.map(p=>p[0]).join(",")}&longitude=${points.map(p=>p[1]).join(",")}&current=temperature_2m,apparent_temperature&timezone=Europe%2FParis`;
    let json=await fetch(url).then(r=>r.json());if(!Array.isArray(json))json=[json];pressureData.temperatures=json;
    pressureData.temperaturePoints=json.map((d,i)=>({lat:points[i][0],lon:points[i][1],name:points[i][2],temperature:d.current?.temperature_2m,apparent:d.current?.apparent_temperature,time:d.current?.time}));
