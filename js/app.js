...
function showNuisancesAt(lon, lat) {
  ...
  const noiseHtml =
    roadDb || railDb
      ...
      : "";
  const commune=communeAt(lon,lat),code=String(commune?.properties?.code||commune?.properties?.insee||commune?.properties?.INSEE_COM||""),air=state.stats[code];
  const airHtml=air?`<h3>Qualité de l’air & bruit · commune</h3><div class="kpi-grid"><div class="kpi-tile"><small>Air dégradé</small><strong>${fmt(air.air_degrade_pct,1)} %</strong><em>de la population communale</em></div><div class="kpi-tile warn"><small>Bruit dégradé</small><strong>${fmt(air.bruit_degrade_pct,1)} %</strong><em>de la population communale</em></div></div>`:"";
  openDetail(
    `<span class="detail-tag">DIAGNOSTIC · CE POINT</span><h2>${esc(air?.nom||"Zone sélectionnée")}</h2>${regHtml}${noiseHtml}${airHtml}<p class="flag-note">Les informations affichées conservent leurs unités et millésimes d’origine. Elles ne sont pas fusionnées dans un score artificiel.</p>`,
  );
}
...
async function loadTemperatures(){
  try{
    ...
    json.forEach((d,i)=>{const p=points[i],t=d.current?.temperature_2m;const icon=L.divIcon({className:"",html:`<div class="temp-marker">${fmt(t,1)} °C</div>`,iconSize:[58,28]});const marker=L.marker([p[0],p[1]],{icon,bubblingMouseEvents:false}).addTo(state.layers.temperature);marker.on("click",()=>openDetail(`<span class="detail-tag">TEMPÉRATURE · OPEN-METEO</span><h2>${p[2]}</h2><div class="kpi-grid"><div class="kpi-tile warn"><small>Température</small><strong>${fmt(t,1)} °C</strong></div><div class="kpi-tile"><small>Ressenti</small><strong>${fmt(d.current?.apparent_temperature,1)} °C</strong></div></div><p>Observation horaire au point de grille le plus proche · ${esc(d.current?.time||"")}</p>`));});
    $("temperatureStatus").textContent=`${json.length} points · observation actuelle`;
  }catch{$("temperatureStatus").textContent="Observation momentanément indisponible"}
}
