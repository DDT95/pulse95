...
).then(([communes, stats, roads, rails, boundary, healthSocial]) => {
  state.stats = stats;
  state.healthSocial = healthSocial.communes;
...
function stressCard(row){const contribution=row.contribution??row.score,impact=row.contribution!=null&&Math.abs(row.contribution-row.score)>.05?`<span class="score-impact">Contribution au cumul ${displayScore(contribution)}</span>`:"";return `<article class="stress-card" style="--stress:${stressColor(row.score||0)}"><header><div><small>${esc(row.name)}</small><strong>${displayScore(row.score)}</strong>${impact}</div><em>${row.score==null?"Non calculable":stressLabel(row.score)}</em></header><div class="stress-evidence"><p><b>MESURE</b><span>${esc(row.value)}</span></p><p><b>RÈGLE / CLASSEMENT</b><span>${esc(row.detail)}</span></p></div><small class="stress-source">${esc(row.source||"Donnée publique")}</small></article>`}
...
async function sampleArtificial(lon,lat){
  // OCS GE « couverture » distingue réellement bâti, surfaces minérales et végétation.
  // On échantillonne un disque de 200 m au lieu d'interpréter la couleur de la carte
  // d'artificialisation, qui classe aussi les jardins résidentiels comme artificialisés.
  try{
    const z=16,n=2**z,tileCache=new Map(),steps=17,radiusM=200;
    const palettes={built:[255,55,122],nonBuilt:[255,145,145],mineral:[255,255,153],composite:[255,55,122],bare:[204,204,204],water:[0,204,242],ice:[166,230,204],leaf:[128,255,0],conifer:[0,166,0],mixed:[128,190,0],shrub:[166,255,128],woody:[230,128,0],grass:[204,242,77],otherVeg:[204,255,204]};
    const hardKeys=new Set(["built","nonBuilt","mineral","composite"]),counts={},samples=[];
    const tilePoint=(lo,la)=>{const fx=(lo+180)/360*n,fy=(1-Math.asinh(Math.tan(la*Math.PI/180))/Math.PI)/2*n;return{x:Math.floor(fx),y:Math.floor(fy),px:Math.max(0,Math.min(255,Math.floor((fx-Math.floor(fx))*256))),py:Math.max(0,Math.min(255,Math.floor((fy-Math.floor(fy))*256)))}};
    const getTile=async(x,y)=>{const key=`${x}/${y}`;if(!tileCache.has(key))tileCache.set(key,(async()=>{const url=`https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=OCSGE.COUVERTURE.2024-2026&STYLE=normal&TILEMATRIXSET=PM&FORMAT=image/png&TILEMATRIX=${z}&TILEROW=${y}&TILECOL=${x}`,blob=await fetch(url).then(r=>{if(!r.ok)throw Error(r.status);return r.blob()}),bmp=await createImageBitmap(blob),c=document.createElement("canvas");c.width=256;c.height=256;const ctx=c.getContext("2d",{willReadFrequently:true});ctx.drawImage(bmp,0,0);return ctx})());return tileCache.get(key)};
    for(let iy=0;iy<steps;iy++)for(let ix=0;ix<steps;ix++){const east=(ix/(steps-1)*2-1)*radiusM,north=(iy/(steps-1)*2-1)*radiusM;if(east*east+north*north>radiusM*radiusM)continue;const la=lat+north/111320,lo=lon+east/(111320*Math.cos(lat*Math.PI/180)),p=tilePoint(lo,la);samples.push({p,atPoint:Math.abs(east)<1&&Math.abs(north)<1})}
    await Promise.all([...new Set(samples.map(s=>`${s.p.x}/${s.p.y}`))].map(k=>{const[x,y]=k.split("/").map(Number);return getTile(x,y)}));
    let hard=0,total=0,pointClass=null;
    for(const s of samples){const ctx=await getTile(s.p.x,s.p.y),d=ctx.getImageData(s.p.px,s.p.py,1,1).data;if(d[3]<80)continue;let best=null,bestD=Infinity;for(const[k,c]of Object.entries(palettes)){const dist=(d[0]-c[0])**2+(d[1]-c[1])**2+(d[2]-c[2])**2;if(dist<bestD){bestD=dist;best=k}}if(bestD>2600)continue;counts[best]=(counts[best]||0)+1;total++;if(hardKeys.has(best))hard++;if(s.atPoint)pointClass=best}
    if(!total)return null;return{atPoint:hardKeys.has(pointClass),share:hard/total,counts,total,method:"OCS GE couverture · disque de 200 m"}
  }catch{return null}
}
async function showNuisancesAt(lon, lat) {
  ...
  const noiseHtml =
    roadDb || railDb
      ...
      : "";
  const commune=communeAt(lon,lat),code=String(commune?.properties?.code||commune?.properties?.insee||commune?.properties?.INSEE_COM||""),air=state.stats[code],hs=state.healthSocial?.[code],ban=await reverseBan(lon,lat),artificial=await sampleArtificial(lon,lat),localAirNoise=sampleAirNoise(latlng);
  ...
  const vegKeys=new Set(["leaf","conifer","mixed","shrub","woody","grass","otherVeg"]),vegShare=artificial?Object.entries(artificial.counts).reduce((n,[k,v])=>n+(vegKeys.has(k)?v:0),0)/artificial.total:null,vegetationScore=vegShare==null?null:Math.min(5,Math.max(0,5*(1-vegShare)));
  const radonScore=hs?.radon_classe!=null?Math.min(5,Math.max(0,(hs.radon_classe-1)*2+.5)):null,nitratesScore=hs?.nitrates!=null?(hs.nitrates>=50?5:hs.nitrates>=25?2.5:.5):null,waterRadonScore=radonScore==null&&nitratesScore==null?null:Math.max(radonScore||0,nitratesScore||0);
  const socialScore=hs?.fdep!=null?Math.min(5,Math.max(0,1.2-hs.fdep*.9)):null;
  ...
  const rows=[
    ...
    {name:"Trafic aérien",score:aircraftScore,value:`${aircraftNear.length} aéronef(s) à moins de 5 km`,why:aircraftNear.length?`${aircraftNear.length} aéronef(s) sont actuellement observés dans le voisinage du point ; la pression augmente avec leur nombre.`:"Aucun aéronef n’est actuellement observé dans un rayon de 5 km.",detail:"1 point de pression par aéronef proche, plafonné à 5",source:`ADSB.lol · positions en direct · ${new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}`},
    ...
    {name:"Radon & eau distribuée",score:waterRadonScore,contribution:0,value:hs?`Radon : classe ${hs.radon_classe??"—"}/3 (${hs.radon_niveau||"—"}, ${fmt(hs.radon_bq_m3)} Bq/m³ estimés) · nitrates ${hs.nitrates!=null?fmt(hs.nitrates,1)+" mg/L":"non mesurés"} · dureté ${hs.durete_eau!=null?fmt(hs.durete_eau)+"°f":"—"} · pH ${hs.ph??"—"}`:"Commune sans mesure disponible",why:hs?`Le potentiel radon (classement géologique IRSN/ASNR par commune) et la teneur en nitrates de l’eau distribuée (dernière analyse du contrôle sanitaire, Hub’Eau) sont les deux indicateurs sanitaires les mieux documentés à cette échelle. Le Val-d’Oise repose sur un bassin sédimentaire à potentiel radon presque partout faible ; la note reflète surtout un éventuel dépassement de nitrates. Indicateur structurel et non instantané : il n’entre pas dans le cumul de la pression globale.`:"Aucune commune identifiée sous ce point, ou données absentes de l’export.",detail:"Radon : classe 1 faible à classe 3 élevé · Nitrates : vigilance dès 25 mg/L, limite réglementaire 50 mg/L",source:"IRSN/ASNR (potentiel radon) · Hub’Eau / Ministère de la Santé (contrôle sanitaire de l’eau) · export data.gouv.fr"},
    {name:"Précarité & accès aux soins",score:socialScore,contribution:0,value:hs?`Indice de défaveur sociale (FDep) ${fmt(hs.fdep,1)} · revenu médian ${hs.revenu_median!=null?fmt(hs.revenu_median)+" €/UC/an":"—"} · chômage ${hs.tx_chomage!=null?fmt(hs.tx_chomage*100,1)+" %":"—"} · accessibilité aux soins ${hs.apl??"—"} consult./an`:"Commune sans donnée disponible",why:hs?`Le FDep (Rey et al., calculé pour l’Île-de-France à partir du revenu, du diplôme, du taux d’ouvriers et des familles monoparentales) situe la commune sur l’échelle régionale de défaveur sociale : plus il est négatif, plus la commune cumule de désavantages socio-économiques, un facteur bien documenté de vulnérabilité face aux autres pressions environnementales. Indicateur structurel et non instantané : il n’entre pas dans le cumul de la pression globale.`:"Donnée absente pour cette commune.",detail:"Note = clamp(1,2 − 0,9 × FDep, 0, 5) · FDep francilien centré sur 0, du plus favorisé (positif) au plus défavorisé (négatif)",source:"Insee Filosofi 2021 + indice FDep Île-de-France · data.gouv.fr, DRIEE/ARS Île-de-France 2024"},
    {name:"Végétalisation & résilience",score:vegetationScore,contribution:0,value:vegShare==null?"Lecture indisponible":`${fmt(vegShare*100)} % de couverture végétale dans le disque de 200 m`,why:vegShare!=null?`Cette note traduit l’absence de végétation plutôt qu’une nuisance directe : un point bien végétalisé bénéficie d’un ombrage et d’un rafraîchissement local qui atténuent l’îlot de chaleur et les autres pressions ; elle complète l’artificialisation en isolant la part réellement arborée ou enherbée (bois, haies, pelouses) parmi les surfaces non bâties. Facteur protecteur et non une pression : il n’entre pas dans le cumul de la pression globale.`:"Lecture locale indisponible.",detail:"Note = 5 × (1 − part végétalisée) · classes OCS GE arborées, arbustives et herbacées uniquement",source:"OCS GE IGN · couverture du sol 2024-2026 · disque de 200 m"}
  ];
...
function hourlyBarsHtml(hourly,currentHour){
  const hours=Object.keys(hourly).map(Number).sort((a,b)=>a-b),max=Math.max(...hours.map(h=>hourly[h]||0),1);
  return `<div class="hour-bars">${hours.map(h=>`<div class="hour-bar${h===currentHour?" now":""}${PEAK_HOURS.has(h)?" peak":""}" style="--h:${Math.max(4,Math.round((hourly[h]||0)/max*100))}%" title="${h} h : ${fmt(hourly[h]||0)} validations">${h===6||h%3===0?`<em>${h}h</em>`:""}</div>`).join("")}</div>`;
}
function stationCrowdingNote(s,currentHour,nearbyStops){
  const hourly=s.hourly||{},value=hourly[currentHour]||0,max=Math.max(...Object.values(hourly),1),peakHour=Object.keys(hourly).reduce((a,b)=>(hourly[a]||0)>=(hourly[b]||0)?a:b),lineSet=new Set();
  nearbyStops.forEach(st=>st.lines.forEach(l=>lineSet.add(l)));
  const busLines=lineSet.size,isHub=busLines>=4,inPeak=PEAK_HOURS.has(currentHour),ratio=value/max;
  if(!isHub)return"";
...
function openSynthesis() {
  ...
  const noiseRanking = communes
    ...
    .slice(0, 5);
  const hour=Math.max(6,Math.min(23,new Date().getHours())),hourTotals=Array.from({length:18},(_,i)=>i+6).map(h=>(pressureData.stations||[]).reduce((n,s)=>n+(s.hourly?.[h]||0),0)),validations=hourTotals[hour-6]||0,peakValidations=Math.max(...hourTotals,1),activityRatio=validations/peakValidations,activityLabel=activityRatio>=.8?"Très forte":activityRatio>=.5?"Soutenue":activityRatio>=.3?"Intermédiaire":"Calme",temps=(pressureData.temperaturePoints||[]).map(p=>Number(p.temperature)).filter(Number.isFinite),meanTemp=temps.length?temps.reduce((a,b)=>a+b,0)/temps.length:null,planes=movingAircraft.length,trafficText=$("trafficStatus")?.textContent||"Flux Sytadin",trafficLive=!/ancienne|indisponible|hors calcul/i.test(trafficText),noisePct=100*noise/pop,tempPosition=meanTemp==null?50:Math.max(0,Math.min(100,(meanTemp+5)/45*100));
  $("synthesisContent").innerHTML =
    `<div class="synthesis-dashboard-head"><span class="detail-tag">CARTE DE PRESSION ENVIRONNEMENTALE · VAL-D’OISE</span><h2>Le département maintenant</h2><p>${new Date().toLocaleString("fr-FR",{weekday:"long",day:"numeric",month:"long",hour:"2-digit",minute:"2-digit"})} · données actuelles et repères structurels</p></div><div class="synthesis-now"><article class="metric-card metric-transit"><div><small>GARES · ${hour} H–${hour+1} H</small><strong>${activityLabel}</strong><span>${fmt(validations)} validations estimées</span></div><div class="metric-ring" style="--value:${activityRatio*100};--metric:#6a6af4"><b>${fmt(activityRatio*100)}%</b></div><em>du pic journalier</em></article><article class="metric-card metric-traffic"><div><small>TRAFIC ROUTIER</small><strong>${trafficLive?"En direct":"Hors calcul"}</strong><span>${esc(trafficText.replace(/^Sytadin · /,""))}</span></div><div class="live-signal ${trafficLive?"is-live":"is-stale"}"><i></i><i></i><i></i><i></i><i></i></div><em>${trafficLive?"flux actuel":"donnée trop ancienne"}</em></article><article class="metric-card metric-plane"><div><small>TRAFIC AÉRIEN</small><strong>${fmt(planes)} <i>aéronef${planes>1?"s":""}</i></strong><span>positions observées maintenant</span></div><div class="plane-scale"><i style="--value:${Math.min(100,planes/12*100)}%"></i></div><em>échelle visuelle · 0 à 12</em></article><article class="metric-card metric-weather"><div><small>MÉTÉO · 5 POINTS</small><strong>${meanTemp==null?"—":fmt(meanTemp,1)+" °C"}</strong><span>température moyenne actuelle</span></div><div class="temperature-scale"><i style="--position:${tempPosition}%"></i></div><em>froid · confort · chaleur</em></article></div><div class="synthesis-columns"><section class="synthesis-viz noise-overview"><div><span class="section-kicker">BRUITPARIF · 2024</span><strong>Exposition structurelle</strong><small class="synthesis-caption">Population située dans les classes les plus dégradées</small></div><div class="noise-donut" style="--value:${noisePct}"><div><strong>${fmt(noise)}</strong><span>habitants</span></div></div><div class="noise-share"><strong>${fmt(noisePct,1)} %</strong><span>de la population étudiée</span><small>sur ${fmt(pop)} habitants</small></div></section><section class="synthesis-viz ranking-viz"><span class="section-kicker">EFFECTIFS EXPOSÉS</span><strong>Communes les plus concernées</strong><small class="synthesis-caption">Longueur = nombre d’habitants · pastille = part de la commune</small>${rankingBars(noiseRanking,"bruit_degrade_pct","#e65a2f")}</section></div>`;
...
function rankingBars(rows, percentageKey, color) {
  ...
  return rows
    .map(
      (row) =>
        `<div class="synthesis-bar-row"><div><span>${esc(row.nom)}</span><b>${fmt(row.exposed)} hab.</b><em>${fmt(row[percentageKey], 1)} %</em></div><div class="synthesis-bar-track"><i style="--bar-width:${(100 * row.exposed) / maximum}%;--bar-color:${color}"></i></div></div>`,
    )
    .join("");
}
