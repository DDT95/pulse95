...
function stressCard(row){return `<div class="stress-card" style="--stress:${stressColor(row.score||0)}"><div><small>${esc(row.name)}</small><strong>${displayScore(row.score)}</strong><em>${row.score==null?"Non calculable":stressLabel(row.score)}</em></div><span>${esc(row.value)}</span><p>${esc(row.detail)}</p></div>`}
...
function globalStress(scores){const positive=scores.filter(s=>Number.isFinite(s)&&s>0).sort((a,b)=>b-a).slice(0,4);if(!positive.length)return 0;const mean=positive.reduce((a,b)=>a+b,0)/positive.length;return Math.min(5,Math.max(mean,positive[0]-.5))}
...
async function showNuisancesAt(lon, lat) {
  ...
  const mobility5=mobilityScore/20,traffic=sampleTrafficAt(lon,lat),roadScore=Math.max(roadDb?Math.min(5,Math.max(0,(roadDb-40)/7)):0,road[0]?.cat?6-road[0].cat:0,traffic?.label==="saturé"?5:traffic?.label==="ralenti"?3:traffic?0.5:0),railScore=Math.max(railDb?Math.min(5,Math.max(0,(railDb-50)/5)):0,rail[0]?.cat?6-rail[0].cat:0);
  ...
  const rows=[
    {name:"Mobilité & fréquentation",score:mobility5,value:`${stationHits.length} gare(s) · ${busHits.length} arrêt(s)`,detail:stationHits[0]?`${stationHits[0].name} · ${fmt(stationHits[0].weekdayAverage)} validations/jour`:"Aucune gare dans son aire d’influence"},
    {name:"Équipements & services",score:equipmentScore,value:`${equipmentHits.length} lieux à moins de 750 m`,detail:"Nombre brut BPE/OSM, transformation logarithmique"},
    {name:"Pression routière",score:roadScore,value:roadDb?`${roadDb}–${roadDb+5} dB(A)`:traffic?`Trafic ${traffic.label}`:"Pas de niveau local",detail:road[0]?`Secteur réglementaire catégorie ${road[0].cat}${traffic?` · trafic ${traffic.label}`:""}`:"Hors secteur routier classé"},
    {name:"Pression ferroviaire",score:railScore,value:railDb?`${railDb}–${railDb+5} dB(A)`:"Pas de niveau local",detail:rail[0]?`Secteur réglementaire catégorie ${rail[0].cat}`:"Hors secteur ferroviaire classé"},
    {name:"Air & bruit",score:airScore,value:localAirNoise?`Classe locale de coexposition ${localAirNoise}/5`:"Classe locale indisponible",detail:air?`Contexte communal : air ${fmt(air.air_degrade_pct,1)} % · bruit ${fmt(air.bruit_degrade_pct,1)} %`:"Contexte communal indisponible"},
    {name:"Météo & extrêmes",score:climateScore,value:temp?`${fmt(temp.temperature,1)} °C · ressenti ${fmt(temp.apparent,1)} °C`:"Observation indisponible",detail:`${weather.detail}${heatDay?` · aléa morphoclimatique ${heatDay}/5`:""}`},
    {name:"Artificialisation",score:artificialScore,value:artificial==null?"Lecture indisponible":artificial?"Sol artificialisé":"Sol non artificialisé",detail:"OCS GE IGN au point sélectionné · pression de fond limitée"},
    {name:"Trafic aérien",score:aircraftScore,value:`${aircraftNear.length} aéronef(s) à moins de 5 km`,detail:"Positions ADS-B observées maintenant"}
  ];
  ...
  openDetail(`<span class="detail-tag">FICHE DE STRESS TERRITORIAL</span><h2>Point sélectionné · ${esc(air?.nom||ban?.city||"Val-d’Oise")}</h2><p class="point-location"><b>${lat.toFixed(6)}, ${lon.toFixed(6)}</b><br>Adresse BAN la plus proche : ${esc(ban?.label||"indisponible")}</p><div class="stress-global" style="--stress:${stressColor(global)}"><small>Stress territorial</small><strong>${displayScore(global)}</strong><em>${stressLabel(global)}</em></div><div class="stress-grid">${cards}</div><details><summary>Mesures et classements réglementaires</summary>${regHtml}${noiseHtml}</details><p class="flag-note">Synthèse des quatre pressions positives les plus fortes, avec garde-fou sur la pression dominante. Chaque note reste accompagnée de sa mesure ou de son classement. Ce diagnostic exploratoire n’est pas un indicateur sanitaire.</p><div class="stress-actions"><button id="printStress">Ouvrir la fiche PDF</button><button id="exportStress">Exporter les données</button></div>`);
...
function openStressPdf(stress){const {jsPDF}=window.jspdf||{};if(!jsPDF)return;const doc=new jsPDF({unit:"mm",format:"a4"}),c=stressColor(stress.score),rgb=c.match(/\w\w/g).map(x=>parseInt(x,16));doc.setFillColor(...rgb);doc.rect(0,0,210,42,"F");doc.setTextColor(255);doc.setFont("helvetica","bold");doc.setFontSize(10);doc.text("DDT DU VAL-D'OISE · PULSE 95",16,13);doc.setFontSize(21);doc.text("Fiche de stress territorial",16,27);doc.setFontSize(12);doc.text(`${displayScore(stress.score)} · ${stressLabel(stress.score)}`,150,27);doc.setTextColor(22,32,43);doc.setFontSize(13);doc.text(stress.commune||"Point sélectionné",16,53);doc.setFont("helvetica","normal");doc.setFontSize(9);doc.text(`Point exact : ${stress.coordinates.lat.toFixed(6)}, ${stress.coordinates.lon.toFixed(6)}`,16,60);doc.text(`Adresse BAN la plus proche : ${stress.nearestAddress||"indisponible"}`,16,66,{maxWidth:178});doc.autoTable({startY:75,head:[["Critère","Note","Donnée factuelle","Fondement"]],body:stress.criteria.map(r=>[r.name,displayScore(r.score),r.value,r.detail]),styles:{fontSize:8,cellPadding:3,valign:"middle"},headStyles:{fillColor:[0,0,145]},columnStyles:{0:{cellWidth:36},1:{cellWidth:20,fontStyle:"bold"},2:{cellWidth:54},3:{cellWidth:72}},didParseCell:d=>{if(d.section==="body"&&d.column.index===1){const col=stressColor(stress.criteria[d.row.index].score||0).match(/\w\w/g).map(x=>parseInt(x,16));d.cell.styles.textColor=col}}});let y=doc.lastAutoTable.finalY+10;doc.setFont("helvetica","bold");doc.text("Lecture",16,y);doc.setFont("helvetica","normal");doc.setFontSize(8);doc.text("La synthèse retient jusqu’aux quatre pressions positives les plus fortes et préserve l’effet d’une pression dominante. Les valeurs réglementaires et observations restent affichées pour rendre chaque note vérifiable. Indice exploratoire, non sanitaire.",16,y+6,{maxWidth:178});doc.setTextColor(90);doc.text(`Produit le ${new Date(stress.generatedAt).toLocaleString("fr-FR")} · Sources publiques Pulse 95`,16,287);const url=URL.createObjectURL(doc.output("blob"));window.open(url,"_blank","noopener")}
